...
**Descripción**: Componente principal de la página de gestión de categorías. Carga la lista de categorías desde la API y proporciona operaciones CRUD completas.

**Funcionalidades**:
- **Tabla**: Nombre (ES), Slug, Estado (badge activo/inactivo), Fecha de creación, Acciones (Editar, Eliminar)
- **Botón "Nueva categoría"**: Abre `CategoryModal` en modo creación
- **Editar**: Abre `CategoryModal` con datos precargados
- **Eliminar**: Confirmación inline antes de borrar, actualiza la tabla sin recargar la página
- Loading skeleton y empty state

**Endpoints API** (vía `adminCategoriesApi`):
- `GET /api/categories?locale=es&pageSize=100` → Lista de categorías
...
**Descripción**: Componente principal de la página de gestión de componentes de hardware. Proporciona operaciones CRUD sobre los componentes.

**Tabla**: SKU, Nombre (ES), Tipo, Stock (badge rojo si `stock <= minStock`), Precio coste, Acciones

**Endpoints API** (vía `adminComponentsApi`):
- `GET /api/components?pageSize=100` → Lista de componentes
...
**Descripción**: Componente principal de la página de gestión de productos. Proporciona operaciones CRUD sobre productos y acceso a la gestión de opciones de componentes.

**Tabla**: SKU, Nombre (ES), Precio base, IVA (%), Activo, Personalizable, Acciones
...
- `GET /api/products?pageSize=100` → Lista de productos
