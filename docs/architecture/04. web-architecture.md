...
│   │   ├── admin/                  # Componentes del área de administración
│   │   │   ├── AdminGuard.tsx      # Guard: requiere rol Admin
│   │   │   ├── AdminLayout.tsx     # Layout con sidebar de administración
│   │   │   ├── AdminModal.tsx      # Modal reutilizable (max-w-2xl)
│   │   │   ├── AdminContentShell.tsx  # Shell: cabecera, skeleton, empty state, error
│   │   │   ├── AdminPagination.tsx    # Paginación server-side compartida
│   │   │   ├── categories/         # Gestión de categorías
│   │   │   │   ├── CategoriesContent.tsx
│   │   │   │   ├── CategoryModal.tsx
│   │   │   │   └── CategoryImagePanel.tsx
│   │   │   ├── components/         # Gestión de componentes
│   │   │   │   ├── ComponentsContent.tsx
│   │   │   │   └── ComponentModal.tsx
│   │   │   ├── orders/             # Gestión de pedidos (admin)
│   │   │   │   ├── OrdersAdminContent.tsx
│   │   │   │   └── OrderAdminDetailModal.tsx
│   │   │   └── products/           # Gestión de productos
│   │   │       ├── ProductsContent.tsx
│   │   │       ├── ProductModal.tsx
│   │   │       ├── ComponentOptionsPanel.tsx
│   │   │       ├── CategoryAssignPanel.tsx
...
│   │   │   └── ProductJsonLd.tsx
│   │   ├── configurator/           # Editor 3D de producto (overlay modal)
│   │   │   ├── ProductConfigurator.tsx  # Modal principal: estado, precios, carrito
│   │   │   ├── ConfiguratorViewer.tsx   # Canvas Three.js (carga dinámica, sin SSR)
│   │   │   ├── CustomizationPanel.tsx   # Panel lateral: grupos, precios, botón carrito
│   │   │   └── GroupSelector.tsx        # Selector de opciones por grupo + OptionCard
...
│   │   │   ├── admin-categories.ts # Funciones de API admin de categorías + imagen por URL
│   │   │   ├── admin-components.ts # Funciones de API admin de componentes
│   │   │   ├── admin-orders.ts     # Funciones de API admin de pedidos
│   │   │   └── admin-products.ts   # Funciones de API admin de productos + component-options + imágenes por URL
│   │   └── validations/
...
│       └── admin.ts                # Tipos TypeScript (DTOs admin: categorías, componentes, productos, imágenes, pedidos)
├── tests/
...
│   │   └── cart.ts                 # Helpers de reset y mock del carrito
...
│   │   ├── configurator/           # Tests del editor 3D
│   │   │   └── ProductConfigurator.test.tsx
...
│   │   ├── admin/                  # Tests de componentes de administración
│   │   │   ├── AdminGuard.test.tsx
│   │   │   ├── AdminLayout.test.tsx
│   │   │   ├── categories/
│   │   │   │   ├── CategoryModal.test.tsx
│   │   │   │   └── CategoryImagePanel.test.tsx
│   │   │   ├── orders/
│   │   │   │   ├── OrdersAdminContent.test.tsx
│   │   │   │   └── OrderAdminDetailModal.test.tsx
│   │   │   └── products/
│   │   │       ├── ProductModal.test.tsx
│   │   │       └── ProductImagesPanel.test.tsx
│   │   └── ui/                     # Tests de componentes UI
│   │       ├── Modal.test.tsx
│   │       └── Switch.test.tsx
│   ├── app/
│   │   └── admin/                  # Tests de páginas de administración
│   │       ├── layout.test.tsx
│   │       ├── categorias.page.test.tsx
│   │       ├── componentes.page.test.tsx
│   │       ├── pedidos.page.test.tsx
│   │       └── productos.page.test.tsx
...
│
├── /{locale}/admin/categorias       → Gestión de categorías (requiere rol Admin)
├── /{locale}/admin/componentes      → Gestión de componentes (requiere rol Admin)
├── /{locale}/admin/pedidos          → Gestión de pedidos (requiere rol Admin)
├── /{locale}/admin/productos        → Gestión de productos (requiere rol Admin)
│
...
└── /{locale}/[...rest]              → Catch-all → renderiza la página 404 personalizada
```
...
flowchart TD
    ...
    subgraph NavbarUserMenu["Navbar — icono usuario"]
        NB{"¿Autenticado?"}
        NB -->|No| NL["/{locale}/login"]
        NB -->|Sí| ND["Dropdown UserMenu"]
        ND --> DP["/{locale}/perfil"]
        ND --> DO["/{locale}/pedidos"]
        ND --> DS["/{locale}/configuracion"]
        ND --> DA["/{locale}/admin/categorias (solo rol Admin)"]
        ND --> DL["Logout → /"]
    end
...
└── [locale] Layout (layout.tsx)
    ...
    │
    ├── (admin)
    │   ├── Layout (AdminGuard + AdminLayout)
    │   │   → Protección de rol Admin + navegación lateral de administración
    │   │
    │   ├── /admin/categorias   → CategoriesContent
    │   │   → Tabla CRUD de categorías con modal de creación/edición
    │   │
    │   ├── /admin/componentes  → ComponentsContent
    │   │   → Tabla CRUD de componentes con modal de creación/edición
    │   │
    │   ├── /admin/pedidos      → OrdersAdminContent
    │   │   → Tabla de pedidos con filtros por estado y modal de detalle + avance de estado
    │   │
    │   └── /admin/productos    → ProductsContent
    │       → Tabla CRUD de productos con modal de creación/edición y gestión de opciones de componentes
    │
    ...
    │
    ├── not-found.tsx (NotFoundContent)
    │   → Página 404 personalizada con diseño temático sim racing
    │
    └── /[...rest] (page.tsx — catch-all)
        → Captura cualquier ruta inexistente bajo el locale y llama a notFound()
```
...
El grupo `(admin)` sigue el mismo patrón pero añade verificación de rol. Agrupa las páginas de administración bajo un layout compartido sin añadir `/admin` como parte del route group a la URL (las páginas son accesibles bajo `/admin/...` gracias al directorio `admin/` dentro del grupo). Todas las páginas dentro comparten:
1. **`AdminGuard`**: Si el usuario no está autenticado, redirige a `/login`. Si está autenticado pero no tiene el rol `Admin`, redirige a `/`. Aplica el mismo delay de 300ms que `PrivateAreaGuard` para permitir el refresco de token.
2. **`AdminLayout`**: Layout con sidebar de navegación con links a Pedidos, Categorías, Componentes y Productos.
...
**Email de bienvenida** (`POST /auth/register`):
Tras el registro exitoso, el backend envía automáticamente un email de bienvenida al nuevo usuario. El email se genera en el idioma indicado por el parámetro `language` (es/en). El asunto y el contenido HTML (título, cuerpo, botón "Ir a la tienda" / "Visit the store" y pie de página) se adaptan al idioma del usuario.
...
**Email de restablecimiento** (`POST /auth/forgot-password`):
El backend envía el email de restablecimiento en el idioma almacenado en la cuenta del usuario (`User.Language`), no en el idioma de la sesión activa. El email incluye un enlace con token URL-encoded válido durante 24 horas con la forma `{FrontendBaseUrl}/reset-password?token=...&email=...`. El asunto, el cuerpo y el botón ("Restablecer contraseña" / "Reset Password") se adaptan al idioma del usuario.
...
[Overlay full-screen — z-[200], se monta sobre todo lo anterior]
┌─────────────────────────────────────────────────────────────┐
│  ProductConfigurator                                        │
│  Header: icono Box + "Configure: {nombre}" + botón Cerrar  │
├───────────────────────────────────┬─────────────────────────┤
│  ConfiguratorViewer (React Three  │  CustomizationPanel     │
│  Fiber, carga dinámica sin SSR)   │  • Loading skeleton     │
│                                   │  • GroupSelector ×N     │
│  • Placeholder "sin modelo" si    │  • Precio base (+ IVA)  │
│    model3dUrl es null             │  • Extras (modificador) │
│  • Canvas con OrbitControls,      │  • Total                │
│    Environment studio, Bounds     │  • Botón ADD TO CART    │
│  • Visibilidad de objetos GLB     │                         │
│    según selecciones activas      │                         │
└───────────────────────────────────┴─────────────────────────┘
```

**Integración del configurador 3D**:

El botón "PERSONALIZAR" de `ProductInfo` monta el componente `ProductConfigurator` como overlay de pantalla completa (`fixed inset-0 z-[200]`). El configurador solo está disponible si el producto tiene `isCustomizable: true`. Al confirmar la configuración, `onAddToCart` recibe las selecciones y el precio total calculado, añade el ítem al carrito con `configurationJson` y llama a `onClose`.
...
#### Gestión de Categorías (`/{locale}/admin/categorias`)

| Propiedad | Valor |
|-----------|-------|
| Ruta | `src/app/[locale]/(admin)/admin/categorias/page.tsx` |
| Tipo | Server Component (wrapper) + Client Component (`CategoriesContent`) |
| Autenticación | Requerida + rol `Admin` (protegida por `AdminGuard`) |

**Descripción**: Página de administración de categorías. Delega todo el contenido al client component `CategoriesContent`.

---

#### Gestión de Componentes (`/{locale}/admin/componentes`)

| Propiedad | Valor |
|-----------|-------|
| Ruta | `src/app/[locale]/(admin)/admin/componentes/page.tsx` |
| Tipo | Server Component (wrapper) + Client Component (`ComponentsContent`) |
| Autenticación | Requerida + rol `Admin` (protegida por `AdminGuard`) |

**Descripción**: Página de administración de componentes de hardware. Delega todo el contenido al client component `ComponentsContent`.

---

#### Gestión de Productos (`/{locale}/admin/productos`)

| Propiedad | Valor |
|-----------|-------|
| Ruta | `src/app/[locale]/(admin)/admin/productos/page.tsx` |
| Tipo | Server Component (wrapper) + Client Component (`ProductsContent`) |
| Autenticación | Requerida + rol `Admin` (protegida por `AdminGuard`) |

**Descripción**: Página de administración de productos. Delega todo el contenido al client component `ProductsContent`. Incluye gestión de opciones de componentes y asignación de categorías vinculadas a cada producto.

---

#### Gestión de Pedidos (`/{locale}/admin/pedidos`)

| Propiedad | Valor |
|-----------|-------|
| Ruta | `src/app/[locale]/(admin)/admin/pedidos/page.tsx` |
| Tipo | Server Component (wrapper) + Client Component (`OrdersAdminContent`) |
| Autenticación | Requerida + rol `Admin` (protegida por `AdminGuard`) |

**Descripción**: Página de administración de pedidos. Permite al administrador consultar todos los pedidos de la plataforma, filtrarlos por estado y gestionar su avance. Delega todo el contenido al client component `OrdersAdminContent`.

**Características principales**:
- **Solo lectura + avance de estado**: No hay creación ni eliminación de pedidos; el administrador únicamente puede avanzar el estado en el flujo lineal establecido.
- **Filtros de estado**: 6 chips de filtrado (Todos, Pendiente, En proceso, Enviado, Entregado, Cancelado). El filtro activo se pasa como parámetro `Status` al endpoint, lo que garantiza que el `totalCount` y la paginación reflejen solo los pedidos filtrados (filtrado server-side).
- **Modal de detalle**: Al hacer clic en "Ver detalle", se abre `OrderAdminDetailModal` con el detalle completo del pedido y el botón de avance de estado.

**Endpoint API**: `GET /api/admin/orders?Page=&PageSize=&Status=` → `PaginatedResultDto<AdminOrderSummaryDto>`.

---

#### Página 404 (`/{locale}/[...rest]`)

| Propiedad | Valor |
|-----------|-------|
| Ruta | `src/app/[locale]/not-found.tsx` + `src/components/not-found/NotFoundContent.tsx` |
| Catch-all | `src/app/[locale]/[...rest]/page.tsx` |
| Tipo | Client Component |
| Autenticación | No requerida |

**Descripción**: Página de error 404 con diseño temático de sim racing. Se muestra cuando el usuario navega a cualquier URL que no corresponde a ninguna ruta definida bajo el locale activo.

**Mecanismo de activación**: Next.js App Router solo muestra `not-found.tsx` cuando se llama explícitamente a `notFound()`. Para capturar automáticamente todas las rutas inexistentes, existe una ruta catch-all `[...rest]/page.tsx` que invoca `notFound()`, delegando el renderizado al archivo `not-found.tsx` del segmento `[locale]`.

**Contenido**:
- Número "404" de gran tamaño como elemento visual principal.
- Título y descripción localizados (ES/EN) con temática de circuito de carreras.
- Dos CTAs: "Volver al inicio" (`/`) y "Ver productos" (`/productos`).

**Internacionalización**: Las cadenas de texto se gestionan bajo el namespace `notFound` en `messages/es.json` y `messages/en.json`.
...
| Propiedad | Valor |
|-----------|-------|
| Ruta | `src/components/admin/AdminGuard.tsx` |
| Tipo | Client Component |

**Descripción**: Componente de protección para el área de administración. Extiende la funcionalidad de `PrivateAreaGuard` añadiendo verificación del rol `Admin`.

**Comportamiento**:
- Espera a que el store Zustand se hidrate desde `localStorage` (`_hasHydrated`).
- Mientras no hidrata, muestra pantalla de carga.
- Si no está autenticado → espera 300ms y redirige a `/login`.
- Si está autenticado pero no tiene el rol `Admin` → espera 300ms y redirige a `/` (home).
- Si está autenticado y tiene rol `Admin` → renderiza los `children`.

**Props**:

| Prop | Tipo | Descripción |
|------|------|-------------|
| `children` | `ReactNode` | Contenido del área de administración |
...
| Propiedad | Valor |
|-----------|-------|
| Ruta | `src/components/admin/AdminLayout.tsx` |
| Tipo | Client Component |

**Descripción**: Layout visual para las páginas de administración. Proporciona el sidebar de navegación con links a las secciones de gestión. Sigue el mismo diseño que `PrivateAreaLayout`.

**Estructura del sidebar**:
- Logo "SIMRACING**SHOP**" con link a home
- Navegación con iconos (ClipboardList, Tag, Package, ShoppingCart de Lucide)
- Links: Pedidos (`/admin/pedidos`), Categorías (`/admin/categorias`), Componentes (`/admin/componentes`), Productos (`/admin/productos`)
- El link activo se resalta con `bg-racing-red`
...
| Prop | Tipo | Descripción |
|------|------|-------------|
| `children` | `ReactNode` | Contenido de la página (ej: CategoriesContent) |
...
| Propiedad | Valor |
|-----------|-------|
| Ruta | `src/components/admin/AdminModal.tsx` |
| Tipo | Client Component |

**Descripción**: Modal reutilizable para el área de administración. Similar al componente `Modal` de UI pero con ancho máximo mayor (`max-w-2xl`) para acomodar formularios complejos con múltiples campos y pestañas de traducciones.
...
| Propiedad | Valor |
|-----------|-------|
| Ruta | `src/components/admin/AdminPagination.tsx` |
| Tipo | Client Component |

**Descripción**: Componente de paginación server-side compartido por las páginas de administración de categorías, componentes y productos. Muestra botones "Anterior" y "Siguiente" junto con el indicador "Página X de Y". Se oculta automáticamente cuando `totalPages <= 1`.
...
| `onPageChange` | `(page: number) => void` | Callback invocado al cambiar de página |
...
| Propiedad | Valor |
|-----------|-------|
| Ruta | `src/components/admin/categories/CategoriesContent.tsx` |
| Tipo | Client Component |

**Descripción**: Componente principal de la página de gestión de categorías. Carga la lista de categorías desde la API con paginación server-side y proporciona operaciones CRUD completas.

**Funcionalidades**:
- **Tabla**: Nombre (ES), Slug, Estado (badge activo/inactivo), Acciones (Editar, Eliminar)
- **Paginación**: Server-side con 10 elementos por página vía `AdminPagination`. Navega automáticamente a la página anterior al eliminar el último elemento de una página
- **Botón "Nueva categoría"**: Abre `CategoryModal` en modo creación; tras guardar vuelve a la página 1
- **Editar**: Abre `CategoryModal` con datos precargados; tras guardar recarga la página actual
- **Eliminar**: Confirmación inline antes de borrar; recarga la página actual o retrocede si era el último elemento
- Loading skeleton y empty state

**Endpoints API** (vía `adminCategoriesApi`):
- `GET /api/categories?locale=es&pageSize=10&page={n}` → Lista paginada de categorías
...
| Propiedad | Valor |
|-----------|-------|
| Ruta | `src/components/admin/categories/CategoryModal.tsx` |
| Tipo | Client Component |

**Descripción**: Modal de creación/edición de categorías. Usa `AdminModal` como contenedor con pestañas para traducciones ES/EN.

**Campos base**: `isActive` (checkbox)

**Campos por pestaña (ES y EN)**: Nombre, Slug (auto-generado desde el nombre, editable manualmente), Descripción corta

**Slug auto-generación**: El slug se genera automáticamente al escribir el nombre mientras el usuario no lo haya editado manualmente. Una vez editado directamente (`slugTouched = true`), deja de auto-generarse.
...
| `onSaved` | `() => void` | Callback tras guardar exitosamente |
| `editItem` | `CategoryDetailDto \| null` | Si no es null, modo edición |

**Flujo en creación**: Llama `adminCategoriesApi.create(dto)` con traducciones incluidas.

**Flujo en edición**: Llama `adminCategoriesApi.update(id, dto)` + `adminCategoriesApi.updateTranslations(id, translationsDto)`.

**Pestaña "Imagen"** (solo en modo edición): Monta `CategoryImagePanel` para gestionar la imagen de la categoría por URL.
...
| Propiedad | Valor |
|-----------|-------|
| Ruta | `src/components/admin/components/ComponentsContent.tsx` |
| Tipo | Client Component |

**Descripción**: Componente principal de la página de gestión de componentes de hardware. Proporciona operaciones CRUD sobre los componentes con paginación server-side.

**Tabla**: SKU, Nombre (ES), Tipo, Stock (badge verde/rojo según disponibilidad), Acciones

**Paginación**: Server-side con 10 elementos por página vía `AdminPagination`. Carga ambas traducciones (ES y EN) en paralelo para la misma página y las combina por ID.

**Endpoints API** (vía `adminComponentsApi`):
- `GET /api/components?locale=es&pageSize=10&page={n}` → Lista paginada de componentes (ES)
- `GET /api/components?locale=en&pageSize=10&page={n}` → Lista paginada de componentes (EN)
- `DELETE /api/admin/components/{id}` → Elimina un componente
...
| Propiedad | Valor |
|-----------|-------|
| Ruta | `src/components/admin/components/ComponentModal.tsx` |
| Tipo | Client Component |

**Descripción**: Modal de creación/edición de componentes de hardware.

**Campos base**: SKU, Tipo de componente, Stock, Stock mínimo, Lead time (días), Peso (g), Precio coste

**Campos por pestaña (ES y EN)**: Nombre, Descripción
...
| Propiedad | Valor |
|-----------|-------|
| Ruta | `src/components/admin/products/ProductsContent.tsx` |
| Tipo | Client Component |

**Descripción**: Componente principal de la página de gestión de productos. Proporciona operaciones CRUD sobre productos con paginación server-side y acceso a la gestión de opciones de componentes y asignación de categorías.

**Tabla**: SKU, Nombre (ES), Precio base, Estado (badge activo/inactivo), Personalizable (badge), Acciones

**Paginación**: Server-side con 10 elementos por página vía `AdminPagination`.

**Nota**: Al editar un producto existente, el modal incluye una pestaña "Componentes" con `ComponentOptionsPanel` para gestionar las opciones vinculadas y una pestaña "Categorías" con `CategoryAssignPanel` para asignar categorías.

**Endpoints API** (vía `adminProductsApi`, `adminComponentsApi` y `adminCategoriesApi`):
- `GET /api/products?locale=es&pageSize=10&page={n}` → Lista paginada de productos
- `GET /api/components?locale=es&pageSize=200&page=1` → Todos los componentes disponibles para el modal
- `GET /api/categories?locale=es&pageSize=200&page=1` → Todas las categorías disponibles para el modal
...
| Propiedad | Valor |
|-----------|-------|
| Ruta | `src/components/admin/products/ProductModal.tsx` |
| Tipo | Client Component |

**Descripción**: Modal de creación/edición de productos. El más complejo de los modales de administración.

**Campos base**: SKU, Precio base, IVA (%), Días de producción, Peso (g), URL modelo 3D, IsActive, IsCustomizable

**Campos por pestaña (ES y EN)**: Nombre, Slug (auto-generado), Descripción corta, Descripción larga, MetaTitle, MetaDescription

**Pestaña "Componentes"** (solo en modo edición): Monta `ComponentOptionsPanel` para gestionar las opciones de componentes vinculados al producto.

**Pestaña "Categorías"** (solo en modo edición): Monta `CategoryAssignPanel` para asignar o quitar categorías del producto. Las categorías disponibles se pasan como prop desde `ProductsContent`.

**Pestaña "Imágenes"** (solo en modo edición): Monta `ProductImagesPanel` para gestionar las imágenes del producto por URL.

**Carga de traducciones**: Al abrir en modo edición, llama `adminProductsApi.getProductBothLocales(editItem.id)` usando el ID (no el slug) para obtener ambas traducciones correctamente.
...
| Propiedad | Valor |
|-----------|-------|
| Ruta | `src/components/admin/products/ComponentOptionsPanel.tsx` |
| Tipo | Client Component |

**Descripción**: Panel de gestión de opciones de componentes dentro del modal de edición de producto. Permite vincular componentes al producto configurando grupos, precios modificadores y opciones por defecto.

**Tabla de opciones**: Componente SKU, Grupo, Precio modificador, Por defecto, Orden, Acciones (Editar, Eliminar)

**Acciones**:
- **Añadir componente**: Formulario inline para seleccionar componente y configurar parámetros
- **Editar opción**: Modifica los parámetros de una opción existente
- **Eliminar opción**: Elimina la vinculación del componente con el producto

**Endpoints API** (vía `adminProductsApi`):
- `GET /api/admin/products/{id}/component-options` → Lista de opciones
- `POST /api/admin/products/{id}/component-options` → Añade opción
- `PUT /api/admin/products/{id}/component-options/{optId}` → Actualiza opción
- `DELETE /api/admin/products/{id}/component-options/{optId}` → Elimina opción
...
| `availableComponents` | `AdminComponentListItem[]` | Componentes disponibles para seleccionar |
...
| Propiedad | Valor |
|-----------|-------|
| Ruta | `src/components/admin/products/CategoryAssignPanel.tsx` |
| Tipo | Client Component |

**Descripción**: Panel de asignación de categorías dentro del modal de edición de producto. Muestra las categorías actualmente asignadas como chips eliminables y una sección de búsqueda para añadir nuevas desde las disponibles. La operación es de reemplazo completo: cada cambio (añadir o quitar) envía la lista completa actualizada de IDs al backend.

**Sección "Categorías asignadas"**: Chips con el nombre de la categoría y botón × para desvincularla. Si no hay ninguna, muestra el mensaje "Sin categorías asignadas".

**Sección "Añadir categoría"**: Campo de búsqueda por nombre o slug que filtra las categorías activas no asignadas. Cada resultado muestra nombre y slug con un botón "Añadir". Si no quedan categorías disponibles, muestra "No hay más categorías disponibles".

**Comportamiento**:
- Al montar, carga las categorías asignadas vía `GET /api/admin/products/{id}/categories`
- Las categorías inactivas (`isActive = false`) se excluyen de la lista de disponibles
- Añadir o quitar una categoría llama a `PUT /api/admin/products/{id}/categories` con la lista completa resultante
- Los errores de API se muestran en un banner de error dentro del panel

**Endpoints API** (vía `adminProductsApi`):
- `GET /api/admin/products/{id}/categories` → Categorías asignadas al cargar
- `PUT /api/admin/products/{id}/categories` → Reemplaza la lista completa al añadir o quitar
...
|------|------|-------------|
| `productId` | `string` | ID del producto |
| `availableCategories` | `CategoryListItem[]` | Todas las categorías del catálogo (activas e inactivas) |
...
| Propiedad | Valor |
|-----------|-------|
| Ruta | `src/components/admin/products/ProductImagesPanel.tsx` |
| Tipo | Client Component |

**Descripción**: Panel de gestión de imágenes por URL dentro del modal de edición de producto. Permite añadir imágenes externas sin subida de archivo y eliminar las existentes.

**Lista de imágenes**: Muestra URL (truncada), alt text y botón "Eliminar" por cada imagen. Si no hay imágenes, muestra el mensaje "Sin imágenes añadidas".

**Formulario de añadir**: Campo URL (requerido), campo alt text (opcional). El `displayOrder` se asigna automáticamente como `images.length` (se añade al final). Tras guardar, la nueva imagen aparece en la lista y el formulario se limpia.

**Eliminación**: Solicita confirmación inline (botones "Sí" / "No") antes de llamar a la API. Tras confirmar, la imagen desaparece de la lista.

**Endpoints API** (vía `adminProductsApi`):
- `GET /api/admin/products/{id}/images` → Lista de imágenes al montar
- `POST /api/admin/products/{id}/images/url` → Añade imagen por URL
- `DELETE /api/admin/products/{id}/images/{imageId}` → Elimina imagen
...
| Propiedad | Valor |
|-----------|-------|
| Ruta | `src/components/admin/categories/CategoryImagePanel.tsx` |
| Tipo | Client Component |

**Descripción**: Panel de gestión de la imagen por URL de una categoría dentro del modal de edición. La categoría admite una única imagen opcional. Permite establecerla o reemplazarla por URL y eliminarla.

**Con imagen**: Muestra URL, alt text y el botón "Reemplazar imagen". Al hacer clic en "Reemplazar imagen" se muestra el formulario para introducir la nueva URL, sustituyendo la anterior en base de datos.

**Sin imagen**: Muestra el formulario "Añadir imagen por URL" con campo URL (requerido) y campo alt text (opcional). Tras guardar, la imagen aparece en la sección de imagen actual.

**Eliminación**: Solicita confirmación inline (botones "Sí" / "No") antes de llamar a la API. Tras confirmar, vuelve al estado "Sin imagen asignada".

**Endpoints API** (vía `adminCategoriesApi`):
- `GET /api/admin/categories/{id}/image` → Imagen actual al montar
- `PUT /api/admin/categories/{id}/image/url` → Establece o reemplaza imagen por URL
- `DELETE /api/admin/categories/{id}/image` → Elimina imagen
...
| Propiedad | Valor |
|-----------|-------|
| Ruta | `src/components/admin/orders/OrdersAdminContent.tsx` |
| Tipo | Client Component |

**Descripción**: Componente principal de la página de gestión de pedidos del administrador. Carga todos los pedidos de la plataforma de forma paginada, permite filtrarlos por estado y abre el modal de detalle al hacer clic en un pedido.

**Estado interno**:
- `orders`: Array de pedidos de la página actual (`AdminOrderSummaryDto[]`).
- `fetchStatus`: Estado de carga (`"loading"` | `"error"` | `"success"`).
- `page` / `totalPages`: Paginación server-side (20 elementos por página).
- `activeFilter`: Filtro activo (`undefined` | `OrderStatus`). Al cambiar, resetea `page` a 1.
- `selectedOrderId`: ID del pedido abierto en el modal de detalle (`string | null`).

**Tabla de pedidos**:

| Columna | Campo |
|---------|-------|
| N° Pedido | `orderNumber` (fuente monospace) |
| Usuario | `userEmail` (`—` si null) |
| Fecha | `createdAt` formateada |
| Estado | `<OrderStatusBadge />` |
| Total | `totalAmount` formateado en € |
| Artículos | `itemCount` |
| Acciones | Botón "Ver detalle" |

**Filtros de estado**: Chips con las opciones Todos / Pendiente / En proceso / Enviado / Entregado / Cancelado. Al cambiar el filtro se pasa el valor `Status` al endpoint (`undefined` = sin filtro). El filtro es server-side: afecta al `totalCount` y a la paginación.

**Estado vacío con filtro activo**: Muestra un mensaje y un enlace "Ver todos los pedidos" que limpia el filtro.

**Paginación**: Usa `AdminPagination` (botones Anterior/Siguiente + indicador de página). Se oculta cuando `totalPages <= 1`.

**Endpoints API** (vía `adminOrdersApi`):
- `GET /api/admin/orders?Page=&PageSize=20&Status=` → Lista paginada de pedidos
...
| Propiedad | Valor |
|-----------|-------|
| Ruta | `src/components/admin/orders/OrderAdminDetailModal.tsx` |
| Tipo | Client Component |

**Descripción**: Modal de detalle de un pedido para el administrador. Carga el pedido completo por ID al montarse, muestra todos sus datos y permite avanzar el estado al siguiente paso en el flujo lineal (`pending → processing → shipped → delivered`).

**Visibilidad**: Cuando `orderId` es `null`, el componente no renderiza nada. Cuando recibe un `orderId` válido, abre el modal y realiza la petición a la API.

**Estados internos**:
- `order`: Datos completos del pedido (`AdminOrderDetailDto | null`).
- `loading`: Skeleton animado durante la carga inicial.
- `error`: Mensaje de error si la carga falla.
- `updating`: Deshabilita el botón de avance durante la llamada a `updateStatus`.
- `updateError`: Mensaje de error si falla el avance de estado.

**Layout** (dos columnas):
- **Cabecera**: número de pedido, fecha, email del usuario y badge de estado (`OrderStatusBadge`).
- **Columna izquierda**: Lista de `orderItems` con nombre, SKU, configuración personalizada (si `configurationJson` no es null, se parsea y muestra como pares `clave: valor` separados por `•`), cantidad y precio total de línea.
- **Columna derecha**: Dirección de envío completa + resumen económico (subtotal, IVA, envío, total).

**Botón de avance de estado**: Solo visible si el pedido no está en estado terminal (`delivered` o `cancelled`). El texto varía según el estado actual:

| Estado actual | Texto del botón |
|---------------|----------------|
| `pending`     | Marcar como En proceso |
| `processing`  | Marcar como Enviado |
| `shipped`     | Marcar como Entregado |
| `delivered` / `cancelled` | *(sin botón)* |

**Al avanzar con éxito**: Actualiza `order` con los datos devueltos por la API y llama `onOrderUpdated` para actualizar la fila correspondiente en la tabla de `OrdersAdminContent`.

**Endpoints API** (vía `adminOrdersApi`):
- `GET /api/admin/orders/{id}` → Detalle completo al montar
- `PATCH /api/admin/orders/{id}/status` → Avanzar estado

**Props**:

| Prop | Tipo | Descripción |
|------|------|-------------|
| `orderId` | `string \| null` | ID del pedido a mostrar. `null` = modal cerrado |
| `onClose` | `() => void` | Callback al cerrar el modal |
| `onOrderUpdated` | `(updated: AdminOrderSummaryDto) => void` | Callback al avanzar estado correctamente |
...
3. **CTAs**: Dos botones en layout flex-wrap con gap.
   ...
   - **Secundario ("VER VÍDEO")**: Botón outline con borde blanco y hover sobre fondo semi-transparente. Al hacer clic abre el `VideoModal`. Localizado desde `home.hero.ctaSecondary`.
...
**Estado local**:
- `videoOpen: boolean` — controla la visibilidad del `VideoModal`. Se inicializa en `false`.
...
| Propiedad | Valor |
|-----------|-------|
| Ruta | `src/components/home/VideoModal.tsx` |
| Tipo | Client Component |

**Descripción**: Modal de vídeo "próximamente" que se abre desde el botón "VER VÍDEO" del hero. Muestra un mensaje informando que el vídeo está en producción y, mientras tanto, reproduce un vídeo aleatorio de una lista predefinida de YouTube.

**Lista de vídeos**: 5 IDs de YouTube codificados en la constante `VIDEOS`. Al abrir el modal se selecciona uno al azar; al pulsar "Otro vídeo" se selecciona otro distinto al actual (el vídeo actual siempre queda excluido de la selección).

**Estructura del contenido**:
- **Badge** `PRÓXIMAMENTE` (rojo) + título en la cabecera
- **Descripción**: mensaje explicativo bajo la cabecera
- **Iframe YouTube**: relación de aspecto 16:9 con `autoplay=1&rel=0`. La prop `key={videoId}` fuerza el desmontaje del iframe al cambiar de vídeo, garantizando la recarga
- **Botón "Otro vídeo"**: icono `RefreshCw` + texto. Llama a `getRandomVideo(currentId)`, que filtra el vídeo actual antes de seleccionar al azar

**Formas de cerrar**:
- Botón X en la cabecera
- Clic en el backdrop semitransparente
- Tecla `Escape` (listener en `document`, limpiado al cerrar)
...
**Efectos**:
- `useEffect([isOpen])`: selecciona un nuevo vídeo aleatorio cada vez que el modal se abre (`isOpen` pasa a `true`)
- `useEffect([isOpen, onClose])`: registra/elimina el listener de teclado según el estado del modal

**Claves de traducción** (`home.videoModal`): `badge`, `title`, `description`, `nextVideo`
...
3. **Acciones** (siempre visible):
   - **Search**: `<Link>` que navega a `/productos` con icono de búsqueda. Usa `Link` de next-intl para soporte locale-aware.
   - **User**: Componente `UserMenu`. Si el usuario no está autenticado, renderiza un `<Link>` directo a `/login`. Si está autenticado, renderiza un botón que despliega el dropdown de usuario (ver `UserMenu`).
...
| Propiedad | Valor |
|-----------|-------|
| Ruta | `src/components/layout/UserMenu.tsx` |
| Tipo | Client Component |

**Descripción**: Componente autocontenido que gestiona el icono de usuario en la barra de navegación. Implementa comportamiento condicional según el estado de autenticación: enlace a login si no hay sesión, o dropdown con opciones de cuenta si el usuario está autenticado.

**Comportamiento condicional**:
- **Sin sesión**: Renderiza un `<Link>` a `/login` con el icono `User` y `aria-label` del namespace `nav.account`.
- **Con sesión**: Renderiza un botón toggle que despliega un panel dropdown con las opciones del usuario.

**Dropdown** (solo visible cuando autenticado y abierto):

```
┌──────────────────────────────┐
│  Nombre Apellido             │  ← font-semibold
│  usuario@email.com           │  ← text-silver
├──────────────────────────────┤
│  👤  Mi Perfil               │  → /perfil
│  📦  Mis Pedidos             │  → /pedidos
│  ⚙️  Configuración           │  → /configuracion
│  🔧  Administración          │  → /admin/categorias  (solo rol Admin)
│ ──────────────────────────── │
...
**Estado interno**:
- `isOpen: boolean` — Controla la visibilidad del dropdown.

**Interacciones de cierre**:
- Clic en el botón toggle (segundo clic).
- Clic fuera del componente (`mousedown` en document).
- Tecla `Escape`.
- Clic en cualquier enlace del menú.

**Lógica de nombre de usuario** (campo `displayName`):
1. Si `firstName` y/o `lastName` están disponibles → muestra "Nombre Apellido".
2. Si no hay nombre → muestra `user.email` como fallback.

**Flujo de logout**:
1. Cierra el dropdown inmediatamente.
2. Llama `authApi.logout()` (petición al backend + limpia tokens de localStorage).
3. Llama `store.logout()` (limpia el estado Zustand).
4. Redirige a `/` con `router.push("/")`.

**Accesibilidad**:
- Botón toggle: `aria-expanded`, `aria-haspopup="menu"`.
- Contenedor del dropdown: `role="menu"`.
- Cada opción (link o botón): `role="menuitem"`.

**Enlace de Administración**: Si `user.roles.includes("Admin")`, muestra un item adicional "Administración" que enlaza a `/admin/categorias`. Este item solo es visible para usuarios con rol `Admin`.

**Dependencias**:
- `useIsAuthenticated`, `useUser`, `useAuthStore` — del store de autenticación.
- `authApi.logout` — del cliente de API de autenticación.
- `useRouter` de `@/i18n/navigation` — para la redirección post-logout.
- Traducciones del namespace `nav.userMenu` (`profile`, `orders`, `settings`, `admin`, `logout`).
...
| Propiedad | Valor |
|-----------|-------|
| Ruta | `src/components/configurator/ProductConfigurator.tsx` |
| Tipo | Client Component |

**Descripción**: Overlay modal de pantalla completa (`fixed inset-0 z-[200]`) que orquesta el editor 3D de personalización de producto. Carga las opciones de personalización desde la API, gestiona el estado de selecciones, calcula el precio total en tiempo real y coordina el visor 3D con el panel de opciones.

**Props**:

| Prop | Tipo | Descripción |
|------|------|-------------|
| `product` | `ProductDetail` | Datos del producto a configurar |
| `onClose` | `() => void` | Callback para cerrar el configurador |
| `onAddToCart` | `(selections, totalPrice) => Promise<void>` | Callback al confirmar la configuración |
| `isAddingToCart` | `boolean` | Deshabilita el botón mientras se añade al carrito |

**Layout**:
```
Header: [Box icon] "Configure: {nombre}" ──────── [X botón cerrar]
─────────────────────────────────────────────────────────────
│  ConfiguratorViewer (flex-1)      │  CustomizationPanel (420px)  │
│  · Visor 3D o placeholder        │  · Grupos de opciones         │
│    "sin modelo"                  │  · Desglose de precio         │
│                                  │  · Botón ADD TO CART          │
```

**Estado interno**:
- `groups: CustomizationGroup[]` — Grupos de personalización cargados desde la API.
- `selections: Record<string, string | null>` — Mapa `{nombreGrupo → componentId seleccionado}`. Se inicializa automáticamente con la opción `isDefault && inStock` de cada grupo.
- `isLoading: boolean` — Controla el esqueleto de carga del panel.

**Cálculo de precio**:
```
basePriceWithVat = product.basePrice × (1 + product.vatRate / 100)
totalPrice       = basePriceWithVat + Σ priceModifier de opciones seleccionadas
```

**`canAddToCart`**: `true` solo cuando todos los grupos con `isRequired: true` tienen una selección distinta de `null`.

**Efectos**:
- Carga `getProductCustomizations(product.id, locale)` al montar. Ignora respuestas tardías si el componente se desmonta (`cancelled` flag).
- Escucha `keydown` → `Escape` para llamar `onClose`.
- Establece `document.body.style.overflow = "hidden"` mientras está montado y lo restaura al desmontar.

**Carga dinámica de `ConfiguratorViewer`**: Se usa `next/dynamic` con `{ ssr: false }` para evitar importar Three.js en el servidor.

**Endpoint API**: `GET /components/product/{id}?locale={locale}` → `CustomizationGroup[]` (agrupados en `productsApi.getProductCustomizations`).
...
| Propiedad | Valor |
|-----------|-------|
| Ruta | `src/components/configurator/ConfiguratorViewer.tsx` |
| Tipo | Client Component (solo browser — importado con `next/dynamic { ssr: false }`) |
| Dependencias | React Three Fiber, `@react-three/drei`, Three.js |

**Descripción**: Canvas 3D que renderiza el modelo GLB del producto con materiales PBR realistas, iluminación estudio y controles de órbita. Muestra u oculta objetos del GLB en función de las selecciones activas del configurador.

**Props**:

| Prop | Tipo | Descripción |
|------|------|-------------|
| `modelUrl` | `string` | URL del archivo GLB |
| `groups` | `CustomizationGroup[]` | Grupos de personalización |
| `selections` | `Record<string, string \| null>` | Selección activa por grupo |

**Subcomponentes internos**:

- **`Model`**: Carga el GLB con `useGLTF`, lo clona (para no mutar la caché), normaliza la escala al bounding box, aplica overrides de materiales y controla la visibilidad de objetos según `selections`. Se memoiza con `useMemo` sobre la URL para evitar re-clonaciones innecesarias; la visibilidad se actualiza en un `useEffect` separado que reacciona a `selections`.

- **`CanvasLoader`**: Spinner animado (`animate-spin`) con texto "Loading 3D model..." mostrado en el centro del canvas mientras el modelo está en suspense.

**Sistema de materiales** (`MATERIAL_OVERRIDES`): Diccionario de overrides indexado por el nombre exacto del material en el GLB. Cada override puede definir `color`, `roughness`, `metalness`, `envMapIntensity` (→ `MeshStandardMaterial`) y propiedades avanzadas como `clearcoat`, `anisotropy`, `iridescence` (→ activan `MeshPhysicalMaterial` automáticamente). Incluye una textura procedural de fibra de carbono twill generada con Canvas 2D.

**Configuración del Canvas**:
- Cámara: `position [0, 0, 5]`, `fov 50`.
- Tone mapping: `AgXToneMapping` con exposición 1.0.
- Iluminación: `ambientLight` (0.3) + 2× `directionalLight` con sombras.
- Entorno: `Environment preset="studio"` de `@react-three/drei`.
- Controles: `OrbitControls` sin pan, `minDistance 0.2`, `maxDistance 20`.
- Auto-encuadre: `Bounds fit margin={1.4}` alrededor del modelo.
...
| Propiedad | Valor |
|-----------|-------|
| Ruta | `src/components/configurator/CustomizationPanel.tsx` |
| Tipo | Client Component |

**Descripción**: Panel lateral derecho del configurador. Muestra los grupos de personalización con scroll, el desglose de precio y el botón de añadir al carrito.

**Props**:

| Prop | Tipo | Descripción |
|------|------|-------------|
| `groups` | `CustomizationGroup[]` | Grupos de opciones a mostrar |
| `selections` | `Record<string, string \| null>` | Selecciones actuales |
| `onSelectionChange` | `(groupName, componentId) => void` | Callback al cambiar una opción |
| `basePrice` | `number` | Precio base con IVA aplicado |
| `totalPrice` | `number` | Precio total con modificadores |
| `isLoading` | `boolean` | Muestra esqueleto si `true` |
| `canAddToCart` | `boolean` | Habilita/deshabilita el botón de carrito |
| `isAddingToCart` | `boolean` | Deshabilita el botón mientras se procesa |
| `onAddToCart` | `() => void` | Callback al pulsar el botón |

**Estados visuales**:
- `isLoading = true`: 3 esqueletos animados (`animate-pulse`) representando grupos con opción grid 3×.
- `groups.length === 0`: Mensaje centrado "This product has no customization options configured".
- Normal: Lista de `GroupSelector` con scroll + footer sticky.

**Footer sticky** (siempre visible al fondo del panel):
- Desglose de precio: Base price, Extras (solo si `totalPrice > basePrice`), Total.
- Aviso "Please select all required options to continue" si `!canAddToCart`.
- Botón "ADD TO CART" (`bg-racing-red`, deshabilitado si `!canAddToCart || isAddingToCart`).
...
| Propiedad | Valor |
|-----------|-------|
| Ruta | `src/components/configurator/GroupSelector.tsx` |
| Tipo | Client Component |

**Descripción**: Sección de personalización para un grupo de opciones. Muestra el nombre del grupo, su badge de requerido/opcional, y un grid de tarjetas de opción (`OptionCard`). Los grupos opcionales incluyen una tarjeta "None" (`∅`) para deseleccionar.

**Props**:

| Prop | Tipo | Descripción |
|------|------|-------------|
| `group` | `CustomizationGroup` | Grupo con sus opciones |
| `selectedId` | `string \| null` | ID del componente seleccionado actualmente |
| `onChange` | `(componentId: string \| null) => void` | Callback al seleccionar una opción |

**`OptionCard`** (subcomponente interno):

| Elemento | Descripción |
|----------|-------------|
| Thumbnail | Imagen 64×64 con `next/image` o icono `Package` si no hay URL |
| Checkmark | Badge `racing-red` en esquina inferior derecha cuando está seleccionado (`aria-pressed`) |
| Nombre | Texto en `text-white` (seleccionado) o `text-silver` |
| Precio | Modificador de precio en `text-electric-blue` (solo si `priceModifier ≠ 0`) |
| Sin stock | Badge "Out of stock" en la parte superior + `disabled` + opacidad 40% |

Las tarjetas usan `aria-pressed` para accesibilidad y están deshabilitadas (`disabled`) cuando `inStock = false`.
...
**Email de confirmación de pedido** (`POST /orders`):
Tras la creación exitosa del pedido, el backend envía un email de confirmación de forma asíncrona (fire-and-forget, no bloquea la respuesta). El idioma del email se extrae del claim `language` del JWT del usuario autenticado, con fallback a `"es"`. El email incluye el número de pedido, el importe total y un botón con enlace directo al detalle del pedido (`/{locale}/pedidos/{id}`). El asunto ("Confirmación de pedido" / "Order Confirmation"), el cuerpo y las etiquetas se adaptan al idioma del usuario.
